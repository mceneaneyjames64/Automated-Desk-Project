...
async def set_preset_one(client):
    ...
    preset_positions[1] = current_positions.copy()
    save_presets()
    print(f"Preset 1 saved: {preset_positions[1]}")

async def set_preset_two(client):
    print("Set Preset Two...")
    preset_positions[2] = current_positions.copy()
    save_presets()
    print(f"Preset 2 saved: {preset_positions[2]}")

async def set_preset_three(client):
    print("Set Preset Three...")
    preset_positions[3] = current_positions.copy()
    save_presets()
    print(f"Preset 3 saved: {preset_positions[3]}")
...
async def mqtt_command_handler():
    ...
    async with Client(MQTT_BROKER, port=MQTT_PORT, username=MQTT_USERNAME, password=MQTT_PASSWORD) as client:
        ...
        async for message in client.messages:
            ...
            # Recieving position data
            if payload.startswith("Feedback1"):
                full = payload
                processed = full[11:]
                current_positions[1] = processed
                print(f"M1: {processed}")
                
            elif payload.startswith("Feedback2"):
                full = payload
                processed = full[11:]
                current_positions[2] = processed
                print(f"M2: {processed}")
                
            elif payload.startswith("Feedback3"):
                full = payload
                processed = full[11:]
                current_positions[3] = processed
                print(f"M3: {processed}")
            ...
            # One-shot commands
                ...
                asyncio.create_task(ONE_SHOT_COMMANDS[payload](client))
            #else:
...
async def heartbeat_monitor():
    ...
    while True:
        ...
        if last_heartbeat_time is None:
            # No heartbeat ever received yet; wait silently at startup
            await asyncio.sleep(1)
            continue
        if now - last_heartbeat_time > HEARTBEAT_TIMEOUT:
            print("ERROR: No 'Heartbeat' message received in the last 25 seconds!")
...
async def send_and_wait(client, command_payload: str, expected_ack: str, timeout=10, retries=3):
    ...
    await client.publish(MQTT_TOPIC, command_payload)
...
async def run_preset_sequence(client, preset_number: int):
    ...
    async with preset_lock:
        ...
        for motor, position in sequence:

            move_cmd = f"{motor}-move-{position}"
            expected_ack = f"{motor}_done"

            success = await send_and_wait(client, move_cmd, expected_ack)

            if not success:
                print(f"Stopping preset {preset_number} due to failure.")
                return
