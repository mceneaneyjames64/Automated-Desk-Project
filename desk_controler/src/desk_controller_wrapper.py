...
class DeskControllerWrapper:
    ...
    def _mqtt_on_connect(self, client, userdata, flags, reason, properties):
        """MQTT connection callback."""
        with self.mqtt_lock:
            self.mqtt_connected = True
        self.logger.info(f"✓ MQTT connected (reason code: {reason})")
    ...
    def _mqtt_on_disconnect(self, client, userdata, disconnect_flags, reason_code, properties):
        """MQTT disconnection callback."""
        with self.mqtt_lock:
            self.mqtt_connected = False
        self.logger.warning(f"MQTT disconnected (reason code: {reason_code})")
    ...
    def print_system_status(self):
        ...
        for motor_id, position in status['motor_positions'].items():
            ...
            pos_str = f"{position:.1f} mm" if position is not None else "unknown"
